...
// three screens nobody has ever seen running, since AGO Calendar is not deployed (`20-20`).
const PROOF_SCREEN = UX_GATE_SCREENS[2];
...
test.describe("fails-before proof", () => {
  test("no horizontal overflow: an element wider than the viewport fails, removing it passes", async ({ page }) => {
    ...
    // **This proof builds its own clean surface rather than borrowing a product screen.**
    // `ago-console`'s version could assert the page starts clean; here it cannot, because every
    // mobile screen in this console really does overflow - the gate's own first run found it on all
    // eight, and that is a product defect with its own issue, not a fixture problem. A proof staged
    // on a screen that is already red proves nothing about the check, so the page is emptied first.
    await page.evaluate(() => {
    ...
    expect(before.overflowPx, "expected the emptied page to start with no overflow").toBe(0);
  ...
  test("WCAG AA contrast: dark-grey-on-dark-blue fails, restoring the real colours passes", async ({ page }) => {
    ...
    await page.evaluate(() => {
      // Built rather than borrowed, for the same reason as the overflow proof above and one more:
      // `ago-console`'s version reached for its own `.ago-message__bubble`, which is that repository's
      // DOM and not this one's. A proof that names another repo's class silently stops proving
      // anything the moment it is copied - it throws instead, which is at least loud, and is exactly
      // what happened on this gate's first run here.
      const bubble = document.createElement("div");
      bubble.id = "ux-gate-contrast-probe";
    ...
    await page.waitForFunction(() => {
      const bubble = document.getElementById("ux-gate-contrast-probe");
    ...
    await page.evaluate(() => {
      const bubble = document.getElementById("ux-gate-contrast-probe");
      const body = bubble?.querySelector<HTMLElement>("p") ?? null;
