 ...
 * fixture cannot name two different ports by hand-editing one and not the other. This repository's
 * fixed port is `4174` (`.env.ux-gate`), `ago-console`'s is `4173` - different values so both gates'
 * `--strictPort` preview servers can bind at the same time.
 */
